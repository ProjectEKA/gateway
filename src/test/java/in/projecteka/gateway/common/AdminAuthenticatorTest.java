...
class AdminAuthenticatorTest {
    ...
    void returnCallerWithSupportedRolesWhenTokenHasRoles() throws JOSEException {
        ...
        var adminTokenVerifier = new AdminAuthenticator(new JWKSet(rsaKey.toPublicJWK()), clientId);

        Mono<Caller> verify = adminTokenVerifier.verify(format("bearer %s", token));

        create(verify).expectNext(new Caller(clientId, false, roles)).verifyComplete();
    }
    ...
    void returnCallerWithEmptyRolesWhenTokenDoesNotHaveValidRoles() throws JOSEException {
        ...
        create(verify).expectNext(new Caller(clientId, false, List.of())).verifyComplete();
    }
